#!/usr/bin/env python3
"""Render results/benchmark_p95_comparison.png from result_*.json.
...
METRIC_KEY = "p95_ms"
METRIC_LABEL = "P95 latency (ms, log scale)"
TITLE = "Drug-Discovery Graph DB Performance (P95, lower is better)"
OUT_FILE = "benchmark_p95_comparison.png"
...
def main():
    ...
    for i, (eng, label, color) in enumerate(ENGINES):
        ...
        vals = []
        for k in op_keys:
            v = res[eng]["operations"].get(k, {}).get(METRIC_KEY)
